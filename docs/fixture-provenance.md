...
| `tests/detection-features.test.ts`  | Email, media player, Electron, XR, wearable, Android model, Form-Factors refinements, helpers                   | Synthetic product-version combinations authored for UA Compass; MIT original work.                      |
...
| `tests/evidence.test.ts`            | Opt-in matched-rule-id tracing, custom rule ids, immutability                                                   | Independently composed fixtures; MIT original work.                                                     |
| `tests/intent.test.ts`              | Advisory fetch-intent buckets across named crawlers and client types                                            | Synthetic crawler fixtures; intent taxonomy is UA Compass original work keyed on public vendor docs.    |
| `tests/cli.test.ts`                 | CLI subprocess behavior: argv, bounded stdin, evidence, help, exit codes                                        | Synthetic User-Agent strings; shells out to the built `dist/cli.js`.                                    |
...
| [User-Agent Client Hints specification](https://wicg.github.io/ua-client-hints/)                                                                                      | Brand lists, mobile/platform fields, `Sec-CH-UA-Form-Factors` tokens, high-entropy hints, and GREASE behavior. | Format reference only; the form-factor token set (`Watch`, `XR`, `Mobile`, `Tablet`, `Desktop`, `EInk`) is taken from the spec; synthetic values authored locally. |
...
| [Android Compatibility Definition Document (CDD)](https://source.android.com/docs/compatibility/17/android-17-cdd)                                                    | Android WebView UA format (`Android <version>; [<model>] [Build/<build>]`) and `Build.MODEL` meaning.          | Format reference only; bounded model extraction and legacy-locale handling are original code; synthetic fixtures authored locally.                                 |
